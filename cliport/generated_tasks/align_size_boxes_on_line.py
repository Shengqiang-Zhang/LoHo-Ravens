...
class AlignSizedBoxOnLine(Task):
    ...
    def reset(self, env):
        ...
        for i in range(n_box):
            ...
            box_pose = self.get_random_pose(env, box_size+i*increase, bound=np.array([[0.36, 0.75], [-0.5, 0.5], [0, 0.3]]))
        ...
        targ = []
        ini_pose = np.array([0.35, -0.4, 0.02])
        targ.append(ini_pose)
        for i in range(n_box):
            targ.append(np.zeros(3))
            targ[i+1][0] = 0.35
            targ[i+1][1] = targ[i][1] + ((box_size+i*increase)/2)[1] + ((box_size + (i+1)*increase)/2)[1] + 0.1
        ...
        for i in range(n_box):
            self.add_goal(objs=[boxs[i][0]], matches=np.ones((1, 1)), targ_poses=[targ[i]], replace=False,
                          ...
                          language_goal=self.lang_template.format(boxs[i][1]))