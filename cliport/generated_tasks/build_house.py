...
class BuildHouse(Task):
    ...
    def reset(self, env):
        ...
        for _ in range(4):
            block_pose = self.get_random_pose(env, block_size, bound=np.array([[0.25, 0.75], [-0.1, 0.5], [0, 0.3]]))
        ...
        roof_blocks = []
        for _ in range(4):
            block_pose = self.get_random_pose(env, block_size, bound=np.array([[0.25, 0.75], [-0.2, 0.5], [0, 0.3]]))
        ...
        roof_target_poses = [((0.7, -0.25, 0.1),(0,0,0,1)), ((0.6, -0.25, 0.1),(0,0,0,1)), ((0.65,-0.3,0.14),ro), ((0.65,-0.2,0.14),ro)]
        ...
        self.add_goal(objs=roof_blocks[:2], matches=np.ones((2, 2)), targ_poses=roof_target_poses[:2], replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 5,
